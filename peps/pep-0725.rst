...
seven keys. "build-requires", "build-host-requires" and "dependencies"
are for specifying three types of *required* dependencies:

1. ``build-requires``, build tools to run on the build machine
2. ``build-host-requires``, build dependencies needed for the host machine but also needed at build time.
...
``optional-build-host-requires``, ``optional-dependencies``), which have the same role that
...
Cloud services and products that allow their users to install or build Python
packages, and which therefore need the system dependencies of those Python
packages, will also benefit from this metadata. Two examples of such products
are `Read the Docs`_ and `FastAPI Cloud`_. The former currently uses custom
``build.*`` config file settings, and the latter is in development and its team
has indicated an interest in using the design in this PEP in lieu of inventing
its own custom scheme.
...
Improvement to building packages with external dependencies from source may be
particularly relevant to platforms which must rely on cross-compilation like
Android and iOS. PyPI now supports those platforms, and rolling out support for
packages with native dependencies will be significantly easier to handle if a
standardized workflow/tool to cross-build them using the ``[external]``
metadata can be implemented.
...
``build-requires`` and ``build-host-requires`` under ``[external]`` for consistency.
...
unrelated to cross-compiling, it may decide to merge the ``build-host-requires`` list
...
the Central Registry (see :pep:`804`) if it is possible to do so (i.e., the
concrete implementations of the virtual package are able to share the same
versioning scheme).
...
and ``build-host-requires`` content do not need to be reflected in existing core
metadata fields.
...
Effect of vendoring dependencies on wheel metadata
''''''''''''''''''''''''''''''''''''''''''''''''''
...
auditwheel_, delvewheel_ and delocate_ automate this process for dynamically
linked shared libraries. Note that the requirements on making wheels
redistributable through vendoring shared libraries does not change with this
PEP, and that that vendoring process for shared libraries does not affect Core
Metadata content at all because ``build-requires`` and ``build-host-requires``
do not have corresponding Core Metadata fields.

The only case requiring a closer look is when a pure runtime dependency, i.e.
listed in ``external.dependencies``, gets vendored into wheels. Note, there are
no commonly used tools that do this - it could be achieved with custom
scripting though. When this is done, a ``Requires-External-Dep`` entry in an
sdist may be removed from the METADATA file in a wheel built from that sdist.
...
wheel(s) on a public index server, depending on the build/distribution process.
Note that this does not imply that the field must then be marked as ``Dynamic``,
since ``Dynamic`` only applies to wheels built from an sdist by a build
backend (see the note on prebuilt wheels in `the Dynamic specification`_).
...
Public index servers MAY disallow the use of DepURLs that are not present
in the :pep:`804` Central Registry in uploaded distributions.
...
``build-host-requires``/``optional-build-host-requires``
''''''''''''''''''''''''''''''''''''''''''''''''''''''''

- Format: Array of external dependency specifiers (``build-host-requires``) and a
  table with values of arrays of external dependency specifiers
  (``optional-build-host-requires``) -
  `Core metadata`_: N/A

The (optional) external host requirements needed to build the project.

For ``build-host-requires``, it is a key whose value is an array of strings. Each
string represents a host requirement of the project and MUST be formatted as
a valid external dependency specifier.

For ``optional-build-host-requires``, it is a table where each key specifies an
...
.. code:: toml
    ...
    [external.optional-build-host-requires]
    extra = [
      "dep:generic/lcms2",
...
Adding a ``build-host-requires`` key under ``[build-system]``
-------------------------------------------------------------

Adding ``build-host-requires`` for host dependencies that are on PyPI in order to
...
adds a ``build-host-requires`` under the ``[external]`` table. However, it isn't
...
contains more arguments in favor and against adding ``build-host-requires`` under
...
.. _Read the Docs: https://docs.readthedocs.com/platform/stable/
.. _FastAPI Cloud: https://fastapicloud.work/
.. _the Dynamic specification: https://packaging.python.org/en/latest/specifications/core-metadata/#dynamic-multiple-use
