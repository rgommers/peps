...
- If the version field contains operators, this field is moved to the ``vers``
  qualifier. The field value must be prepended with the ``vers:`` scheme and
  the same type used in the URL.
...
A `proposal to add an abstract type <https://github.com/package-url/purl-spec/issues/222>`__
is being discussed for revision 1.1.

In the meantime, we propose adding two new types to our ``dep:`` derivative:
...
Once PURL 1.1 is published and the ``abstract`` type is implemented,
``dep:interface`` and ``dep:compiler`` URLs can be converted to PURLs by
prepending the type ``abstract``.
...
- ``dep:compiler/c`` becomes ``pkg:abstract/compiler/c``.
- ``dep:interface/blas>=3,<4`` becomes
...
``dep:(compiler|interface)/name``, which is analogous to but not part of the PURL spec.
