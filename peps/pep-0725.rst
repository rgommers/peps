...
Impact on core metadata fields
------------------------------

The `core metadata`_ specification contains one relevant field, namely
``Requires-External``. While it would be a good candidate to record the
``external.dependencies`` table, the authors have decided to not use this
field to propagate the external runtime dependencies metadata.

To avoid any confusion in the future, this PEP also proposes to deprecate the
``Requires-External`` field altogether.

The reasons are discussed below.

Insufficient fields in core metadata
''''''''''''''''''''''''''''''''''''
...
- The ``optional-dependencies`` content from ``[external]`` would need to either
  reuse ``Provides-Extra`` or require a new ``Provides-External-Extra`` field.
- The core metadata specification does not contain fields for any metadata in
  ...
  and ``host-requires`` content cannot be reflected in existing core
  metadata fields. This creates an inconsistency in the type of external metadata
  that is included in sdists and wheels. The alternative would be to add new
  fields, resulting in a core metadata version bump.

The authors consider that none of these options are desirable and hence decide
not to leverage core metadata for the ``[external]`` table.
...
``pyproject.toml`` in a wheel would need therefore to be allowed to be dynamic
and different (narrower) than those for the corresponding sdist. Managing this
complexity in build backends and vendoring tools does not seem to be desirable either.
...
deprecation of the ``External-Requires`` core metadata field. Given its low
penetration in the ecosystem (e.g. not supported by mainsteam build backends like
``setuptools``), we estimate the impact to be low.
...
what it's done for obsolete fields like ``Requires``
(deprecated in favor of ``Requires-Dist``).
...
2. The PURL specification, as a key part of this spec, has a Python package
   ...
   but a ``dep``-specific implementation is available in `pyproject-external`_
   too.
