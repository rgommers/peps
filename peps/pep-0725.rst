...
.. JRG: I suggested `dep:` (dependency) but it could also be `req:` (requirement)

- The scheme for this PURL derivative is ``dep:`` (for dependency). 
...
- ``dep:`` URLs can also admit ``interface`` and ``compiler`` types from
  ``virtual:`` PURLs, directly.
- As a direct consequence of the above definitions, ``dep`` URLs can be converted
...
- ``dep:interface/lapack@>=3.7.1``: any package implementing the LAPACK interface
  for version greater or equal than ``3.7.1``.

Note: Our ``dep:`` derivative is not part of the PURL spec because it introduces
