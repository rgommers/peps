...
.. JRG: For completeness, I would also clarify what a *runtime* dependency is.
...
.. JRG: Above we are using target platform. Should it be host platform?
...
.. JRG: Why? are host deps it added implicitly as runtime ones?
...
Note: Adding a new ``abstract/`` type to PURL has been
`proposed <https://github.com/package-url/purl-spec/issues/222>`__ in the
meantime. If that goes through, ``virtual:`` PURLs can be converted to
``pkg:abstract/`` PURLs and understood as equivalent.
...
However, this is not very ergonomic. We propose a PURL-compatible syntax that
extends the ``@`` component to support version ranges directly:

- The scheme for this PURL derivative is ``req`` (for requirement). 
- When no operator is present, the field is understood as a fixed version.
- The version range syntax must be compatible with PEP 440. As a result, it is
  equivalent to ``vers:pypi/`` PURLs.
- ``req:`` URLs can also admit ``interface`` and ``compiler`` types from
  ``virtual:`` PURLs, directly.
- As a direct consequence of the above definitions, ``req`` URLs can be converted
  unambiguously to a ``pkg:`` + ``vers:`` tuple.

Some examples:

- ``req:pypi/numpy@2.0``: ``numpy`` pinned at exactly version 2.0.
- ``req:pypi/numpy@>=2.0``: ``numpy`` with version greater or equal than 2.0.
- ``req:interface/lapack@>=3.7.1``: any package implementing the LAPACK interface
  for version greater or equal than ``3.7.1``.

Note: Our ``req:`` derivative is not part of the PURL spec because it introduces
ambiguities in other ecosystems. PEP 440 style version ranges do not suffer from
this problem and hence why we can overload the ``@`` component without ambiguities.
...
is undesirable. Thus why we propose a joint ``req:`` scheme.

`Ongoing discussions <https://github.com/package-url/purl-spec/issues/386#issuecomment-2633852500>`__
with the PURL specification team are happening to assess the feasibility of
including this idea in the official standard.
