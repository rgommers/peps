...
``Requires-External``. While it would be a good candidate to record the
 ``external.dependencies`` table (runtime dependencies), this PEP proposes
to deprecate this field. The reasons are discussed below.
...
- The core metadata specification does not contain fields for any metadata in
  ...
  and ``host-requires`` content also could not be reflected in existing core
  metadata fields. New ones would need to be added.
...
``pyproject.toml`` in a wheel would need therefore to be allowed to be different
(narrower) than those for the corresponding sdist. Managing this complexity
in build backends and vendoring tools does not seem to be desirable either.
