...
There are cases, in particular when dealing with pre-releases, where
PEP 440 semantics for version comparisons don't quite work. For example,
``1.2.3a`` may indicate a release subsequent to ``1.2.3`` rather than an alpha version.
To handle such cases correctly, it would be necessary to allow arbitrary
versioning schemes. The authors of this PEP consider the added value of
allowing that is not justified by the additional complexity. If desired,
a package author can use either a code comment or the ``qualifier`` field of a
