...
seven keys. "build-requires", "host-requires", "dependencies" are
...
respectively. If present, they will be used by the build machinery, but if absent no error
will be raised. ``[external]``-aware tools will inspect these fields to provision or analyze
the dependency tree as needed, taking this meaning into account (which is specially important
...
useful to provide external dependency metadata for e.g. development or testing, without
polluting the Core Metadata file with unneeded bloat.
