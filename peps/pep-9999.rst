...
system; e.g. ``$XDG_DATA_DIR`` on Linux and others, ``~/Library/Application Support`` on
...
1. Assigning a short identifier to each ecosystem. This is the identifier
   that MUST be used in the mapping filenames mentioned above so they can be
   found in the local filesystem.
2. Reporting the canonical URL for its mapping.
