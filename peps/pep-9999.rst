...
Gentoo follows a similar approach to naming Python packages, using the ``python-dev/``
namespace and some `well-specified rules <https://projects.gentoo.org/python/guide/package-maintenance.html>`__.
...
.. JRG: In this section I'd prefer to speak of ecosystems instead of package managers
...
- A central registry for known generic and virtual PURLs.
- A list of known ecosystems and package managers, where ecosystem maintainers
  can register their name mapping(s).
- A standardized schemsa that defines how mappings should be structured.
- The above documents should be written in a structured, human-readable file format.
  ...
  delivered as JSON, YAML, TOML and/or others. Authors recommend JSON or YAML.
...
1. A central registry of PURLs.
2. A list of known ecosystems and their package manager names.
3. The ecosystem-specific mappings of PURLs to package specifiers,
   plus details of their package managers.
...
canonical location for its mapping. The known package managers are also provided here
so tools can map package manager names back to ecosystems without querying each mapping.

.. JRG: Not sure if tying the package manager list to the known ecosystems list is worth it.
...
.. JRG: Should this file use a XDG-standardized location?
