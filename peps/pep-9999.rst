...
Tools MUST check that the provided identifiers are well formed. They MAY
disallow ``dep:`` identifiers that are not included in the central registry,
and SHOULD recommend using the canonical form instead of a secondary alias.
...
- These details can also be obtained from the actual target ecosystems. In some
  cases this might be even preferable; e.g. for licenses, where downstream packaging
...
With the central registry, tools could choose to reject ``[external]`` metadata
entries that do not conform to the well-known identifiers. We have decided not to
enforce this and instead suggest to simply warn about it.
...
human interaction to review and accept the newly requested identifiers.
