...
ecosystems MUST be maintained by a central authority. The authors propose to:

- Host the ``external-metadata-mappings`` and ``pyproject-external`` repositories under the PyPA_
  Github organization (or equivalent, once PEP 772 is voted).
- Create a maintainers team for these two repositories, seeded with the authors of this PEP and
  regulated by the governance model introduced in PEP 609 or 772.
