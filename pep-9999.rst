...
ecosystem will mean a failed installation. It is thus critically important that each package
ecosystem maintain their mappings. Key to this will be automation. Some ideas for automation
are:
...
undesirable to be loose with PURL definition, because each definition implies maintenance in
...
In an ideal case, these package install commands can be done transparently, and the user 
experience is the same as the self-contained binary wheels of today. There are several
foreseeable issues that will arise, though:
...
If packagers suddenly drop self-contained wheels before the mapping can be well-established,
there will likely be a period of negative user experience. Packages should individually decide for themselves when it is "safe" to stop providing standalone wheels. One metric that might serve this purpose would be fractional download counts. For example, how many of the downloads of the latest version of a project are the self-contained wheels vs. the shared-library variety? This is likely going to be different on each platform.
...
2. A standard specification for a collection of mappings. JSONSchema is widely used for schema in many text editors, and would be a natural choice for expression of the standard specification.
...
A prototype proof of concept implementation was contributed to Grayskull, a conda recipe generator for Python packages, via `conda/grayskull#518 <https://github.com/conda/grayskull/pull/518>`__.
